...
void GLCD_Rectangle(unsigned char x, unsigned char y, unsigned char b, unsigned char a)
{
  ...
  for (j = 0; j < a; j++) {
		glcd_PlotPixel(x, y + j, color);
		glcd_PlotPixel(x + b - 1, y + j, color);
	}
  // rysowanie linii poziomych (podstawy)
  for (j = 0; j < b; j++)	{
		glcd_PlotPixel(x + j, y, color);
		glcd_PlotPixel(x + j, y + a - 1, color);
	}
}
//
...
while(x >= y)
  {
  glcd_PlotPixel(cx+x, cy+y, color); 
  glcd_PlotPixel(cx-x, cy+y, color); 
  glcd_PlotPixel(cx-x, cy-y, color);
  glcd_PlotPixel(cx+x, cy-y, color); 
  glcd_PlotPixel(cx+y, cy+x, color); 
  glcd_PlotPixel(cx-y, cy+x, color); 
  glcd_PlotPixel(cx-y, cy-x, color); 
  glcd_PlotPixel(cx+y, cy-x, color); 
  y++;
...
glcd_PlotPixel(X1,Y1, color); // stawiamy pierwszy krok (zapalamy pierwszy piksel)
  ...
  if (Dy <= Dx) // je?li tak, to idziemy "po iksach"
    ...
    do // ruszamy w drog?
      ...
      if(TwoDxAccumulatedError > Dx)  // je?li TwoDxAccumulatedError jest wi?kszy od Dx
       ...
       glcd_PlotPixel(CurrentX,CurrentY, color);// stawiamy nast?pny krok (zapalamy piksel)
      ...
      do 
        ...
        if(TwoDyAccumulatedError>Dy) 
         ...
         glcd_PlotPixel(CurrentX,CurrentY, color); 
         }while (CurrentY != Y2);
    }
  }
}
